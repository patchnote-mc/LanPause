...
* **Mod Loader:** Fabric (Loader `0.19.3`, API `0.153.0+26.2`, Loom `1.17-SNAPSHOT`)
...
* `mc_decompiled/sources/26.2/common_src/` and
  `mc_decompiled/sources/26.2/client_src/` → Decompiled Minecraft
  26.2 source (Mojang mappings), produced by CFR from
  `sources/26.2/common.jar` and `client.jar`.
  NOTE: both `*_src/` directories are gitignored and regenerated by running
  `mc_decompiled/setup.sh 26.2` (downloads CFR to `mc_decompiled/cfr.jar`,
  then decompiles the jars). They are NOT produced by `./gradlew genSources`.
* `fabric_decompiled/src/` → Fabric API + Fabric Loader source. This is a **git
  submodule** of <https://github.com/FabricMC/fabric-api.git>, pinned to tag
  `0.153.0+26.2`. Initialize/update it via `fabric_decompiled/setup.sh`
...
**Keep docs from going stale:** every doc that tracks the codebase (this file
and the index/knowledge caches) carries a date. When you change
